...
-- Parameters:
--   %(eval_date)s             DATE   -- The simulated "today" (CURRENT_DATE for production)
...
-- SIGNAL 4: Peak seasonal baseline (V2-style, hybrid extension)
-- Averages the last 12 months of PEAK days (last 2 of month + first 1 of next)
-- that share the same day-of-week as the target date.
-- Used in compute_hybrid_estimate when is_peak=True and horizon >= 1 to
-- replace the regular seasonal baseline (which excludes peak history).
-- Returns NULL for non-peak target dates — let Python handle the fallback.
peak_seasonal AS (
    SELECT
        d.target_date,
        d.horizon,
        AVG(hist.order_count) AS peak_seasonal_avg
    FROM dates d
    INNER JOIN (
        SELECT
            CAST(sr.shifting_ts + INTERVAL '5 hours, 30 minutes' AS DATE) AS order_date,
        ...
        -- Only target dates that are peak (last 2 of month OR first of month)
        ON DAY(d.target_date) >= DAYOFMONTH(LAST_DAY(d.target_date)) - 1
            OR DAY(d.target_date) = 1
    WHERE
        -- Historical dates must also be peak (same "month_boundary" definition)
        (
            hist.dom >= DAYOFMONTH(LAST_DAY(hist.order_date)) - 1
            OR hist.dom = 1
        )
        -- Same day-of-week as target (month-end surges are DOW-modulated)
        AND hist.dow = DAYOFWEEK(d.target_date)
        -- Settled historical dates only
        AND hist.order_date < d.target_date
        -- Last 12 months of peak-day history
        AND hist.order_date >= DATEADD(MONTH, -12, d.target_date)
    GROUP BY d.target_date, d.horizon
),
...
SELECT
    ...
    -- NULL when target_date is not peak; Python uses regular seasonal as fallback
    pkseas.peak_seasonal_avg            AS peak_seasonal_avg
FROM dates d
...
LEFT JOIN peak_seasonal pkseas
    ON d.target_date = pkseas.target_date AND d.horizon = pkseas.horizon
